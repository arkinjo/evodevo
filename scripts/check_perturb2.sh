...
for denv in 2 {10..100..10}; do
	...
	$pegcov -jsonin pops/${base}_perturb${denv}_${NOISE}_01_001.json.gz \
		-mode=2 -right=e \
		> per/${base}_${denv}_${NOISE}_${i}_pe.dat
	$pegcov -jsonin pops/${base}_perturb${denv}_${NOISE}_01_001.json.gz \
		-mode=2 -right=G \
		> per/${base}_${denv}_${NOISE}_${i}_pG.dat
...
for cc in pe pG; do
	...
	for i in {01..${NIND}}; do
	    echo -n $denv
	    grep "Ali" per/${base}_${denv}_${NOISE}_${i}_${cc}.dat | awk '$2==0 {printf "\t%f\n", $3}'
	done
	...
	for i in {01..${NIND}}; do
	    echo -n $denv
	    fgrep SVal per/${base}_${denv}_${NOISE}_${i}_${cc}.dat | awk '$2==0 {printf "\t%e\t%e\t%e\n", $3, $4, $5}'
	done
    done > ${base}_${cc}_sval1.dat

    # 2nd singular values
    for denv in 2 {10..100..10}; do
	for i in {01..${NIND}}; do
	    echo -n $denv
	    fgrep SVal per/${base}_${denv}_${NOISE}_${i}_${cc}.dat | awk '$2==1 {printf "\t%e\t%e\t%e\n", $3, $4, $5}'
	done
    done > ${base}_${cc}_sval2.dat
...
for denv in 2 {10..100..10}; do
	...
	paste tmp0.dat tmp1.dat | awk '{printf "%d\t%e\t%e\n", $2, $3, $6}' > LSVec1/${base}_${denv}_${NOISE}_${i}.lsvec1
    done 
done
