...
###Readme
#Run this after running gvar.R and PGtraj.R
...
#tiff("../plots/approjhat.tif",width=2250,height=2250,units="px", pointsize=12, res=300)
#boxplot(df.approj_mingvar[bxpindex+1],col=colvec.s[bxpindex],main="Genetic Bottleneck",xlab="Model",ylab="Projected Novel Phenotype",ylim=c(0,1))
#dev.off()

#tiff("../plots/npprojhat.tif",width=2250,height=2250,units="px", pointsize=12, res=300)
#boxplot(df.npproj_mingvar[bxpindex+1],col=colvec.s[bxpindex],main="Genetic Bottleneck",xlab="Model",ylab="Projected Novel Phenotype",ylim=c(0,1))
#dev.off()

tiff("../plots/pprojhat.tif",width=2250,height=2250,units="px", pointsize=12, res=300)
boxplot(df.pproj_mingvar[c(6,7,4,5,8,9,2,3)],col=c("magenta","cyan"),at=boxatvec,xlab="Model",ylab="Projected Phenotype at Genetic Bottleneck",ylim=c(0,1),xaxt="n",cex.lab=1.5)
axis(side = 1, at = axisatvec, labels=c("Full","NoHier","NoDev","NoCue"))
legend("bottomright",legend=c("Ancestral","Novel"),col=c("magenta","cyan"),lty=1,title="Environment")
dev.off()

tiff("../plots/varpprojhat.tif",width=2250,height=2250,units="px", pointsize=12, res=300)
boxplot(df.vars_mingvar[c(6,7,4,5,8,9,2,3)],col=c("magenta","cyan"),at=boxatvec,xlab="Model",ylab="Variance in projected phenotype at Genetic Bottleneck",xaxt="n",cex.lab=1.5)
axis(side = 1, at = axisatvec, labels=c("Full","NoHier","NoDev","NoCue"))
legend("topright",legend=c("Ancestral","Novel"),col=c("magenta","cyan"),lty=1,title="Environment")
dev.off()


tiff("../plots/gprojhat.tif",width=2250,height=2250,units="px", pointsize=12, res=300)
boxplot(df.gproj_mingvar[bxpindex+1],col=colvec.s[bxpindex],xlab="Model",ylab="Projected Genotype at Genetic Bottleneck",ylim=c(0,1),cex.lab=1.5)
dev.off()
