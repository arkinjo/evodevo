...
layers <- "EFGHJP" #Layers present in model
...
for (denv in denvs){
    ...
    df.ali[spdenv] <- ali_in$V2[which(sv1_in$V1==denv)]
    df.sv1[spdenv] <- sv1_in$V2[which(sv1_in$V1==denv)]
    df.psv1[spdenv] <- sv1_in$V3[which(sv1_in$V1==denv)]
    df.Fnorm[spdenv] <- Fnorm_in$V2[which(sv1_in$V1==denv)]
}
...
boxplot(df.sv1[2:ncol(df.sv1)],ylab = sprintf("1st singular value (%s)",pert_vs), col=colvec, ylim=c(0,1), cex.lab=1.5, cex.main=2.0)
#axis()
legend("topright", title="Model", legend=c("Full","NoHier","NoCue","NoDev"), lty=1, col=cols)
dev.off()
