...
for(layers in c("EFGHJP","CEFGHJP")){ #Loop over models
...
tiff(filename=sprintf("../plots/apr_%d-train.png",gen),width=2250,height=2250,units="px", pointsize=12, res=300)
bp.ipr <- boxplot(df.ipr[2:ncol(df.ipr)],col=colvec.l,las=2,ylab="Adaptive plastic response",main=sprintf("Generation %d",gen),ylim=c(-1,1))
#legend("topright",legend=c("Full","NoHier","NoDev"),col=c("orange","limegreen","darkorchid"),lty=c(1,1,1))
dev.off()

tiff(filename=sprintf("../plots/mapr_%d-train.png",gen),width=2250,height=2250,units="px", pointsize=12, res=300)
bp.mipr <- boxplot(df.mipr[2:ncol(df.mipr)],col=colvec.s,las=1, ylab="Adaptive plastic response", main=sprintf("Generation %d",gen))
#legend("topright",legend=c("Full","NoHier","NoDev"),col=c("orange","limegreen","darkorchid"),lty=c(1,1,1))
dev.off()

tiff(filename=sprintf("../plots/vars_%d-train.png",gen),width=2250,height=2250,units="px", pointsize=12, res=300)
bp.vars <- boxplot(df.vars[2:ncol(df.vars)],col=c("magenta","cyan"),main=sprintf("Generation %d",gen),ylab="Variance in projected phenotype",xlab="Model", at = atvec, xaxt="n")
axis(side = 1, at = c(1.5,4.5), labels = models )
